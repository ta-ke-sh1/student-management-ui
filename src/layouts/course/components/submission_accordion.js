import { Accordion, AccordionDetails, AccordionSummary, Paper, TableContainer, TableRow, TableCell, Table, Button, Grid, Box, Stack, Chip } from "@mui/material"
import ExpandMoreIcon from '@mui/icons-material/ExpandMore';
import { useEffect, useState } from "react";
import { downloadFile, fromMilisecondsToDateString, subtractTime } from "../../../utils/utils";
...
export default function SubmmissionAccordion(props) {
    ...
    const fetchAssignment = () => {
        try {
            axios.get(process.env.REACT_APP_HOST_URL + "/submission?id=" + props.course.id + "-" + props.assignment.name + "-" + props.decoded.id).then((res) => {
    ...
    const handleSubmit = () => {
        try {
            ...
            files.forEach((file) => {
                ...
                if (file.size > 20000000) throw "File size could not be more than 20mbs";
            })
            ...
            axios.post(process.env.REACT_APP_HOST_URL + "/submission/submit", formData, {
                headers: {
    ...
    const handleDownloadFile = (index) => {
        downloadFile(process.env.REACT_APP_HOST_URL + submission.path, submission.fileNames[index])
                    ...
                    <TableContainer component={Paper} className="submission-table" elevation={0} >
                        <Table fullWidth>
                            <TableRow>
                                ...
                                <TableCell className="col-2">{submission !== -1 ? "Submitted" : "Not submitted"}</TableCell>
                            </TableRow>
                            <TableRow>
                            ...
                            </TableRow>
                            {
                                submission !== - 1 ?
                                    <>
                                        <TableRow>
                                            <TableCell className="col-1">Last Modified</TableCell>
                                            <TableCell className="col-2">{fromMilisecondsToDateString(submission.date)}</TableCell>
                                        </TableRow>
                                        <TableRow>
                                            <TableCell className="col-1">File Submissions</TableCell>
                                            <TableCell className="col-2" sx={{ width: '100%' }}>
                                                <Stack direction="row" spacing={1}>
                                                    {submission.fileNames && submission.fileNames.map((fileName, index) => {
                                                        return (
                                                            <Chip key={"submission-" + index} label={fileName} onClick={() => handleDownloadFile(index)} />
                                                        )
                                                    })}
                                                </Stack>
                            ...
                            <TableRow>
                                ...
                                <TableCell className="col-2">
                                    <MuiFileInput value={files} multiple={true} onChange={(newValue) => {
                                        console.log(newValue)
                                        setFiles(newValue)
                                    }} />
                                    <p>Maximum 10 files with no more than 20mb size</p>
                                </TableCell>
                            </TableRow>
                            <TableRow>
                                ...
                                <TableCell className="col-2">{
                                    openTime < 0 ?
                                        <Button variant="contained" onClick={handleSubmit}>Submit</Button> : <Button variant="contained" disabled={true}>Not yet open for submission</Button>
                                }</TableCell>
                            </TableRow>
                        </Table>
                    </TableContainer>
