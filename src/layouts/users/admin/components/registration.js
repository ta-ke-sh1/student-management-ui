...
export default function RegistrationAdmin() {
  ...
  const fetchRows = () => {
    let res = [];
    rooms.forEach((room) => {
      res.push(createData(room.id, room.campus, room.building, room.room));
    });
    setRows(res);
  };
        ...
        <Grid item xs={12}>
          <div className="big-widget">
            <div className="campus-list">
              <CustomTable init_count={4} title={"Campus"} rows={rows} headCells={headCells} colNames={["id", "campus", "building", "number"]} handleEdit={handleEdit} handleDelete={handleDelete} />
            </div>
          </div>
        </Grid>
      </Grid>
